# What's in this brain

Every file you've saved, grouped by kind. Claude reads this to find things fast.

Rebuild it any time with `python3 tools/index.py`.

- Files: 5
- Updated: 2026-08-11 11:13 UTC

## Principle

| Name | What it covers | Owner | Updated |
|---|---|---|---|
| [How We Write (example)](principles/EXAMPLE-how-we-write.md) | Our writing is clear, warm, specific, and easy for a busy reader to act on. |  | 2026-08-11 |
| [Writing Rules](principles/writing-rules.md) | The default writing standard for anything a human will read; bans the patterns that make writing sound like AI. |  | 2026-08-11 |

## Playbook

| Name | What it covers | Owner | Updated |
|---|---|---|---|
| [New Client Onboarding (example)](playbooks/EXAMPLE-new-client-onboarding.md) | The steps for moving an approved project from agreement to a confident first week. |  | 2026-08-11 |

## Template

| Name | What it covers | Owner | Updated |
|---|---|---|---|
| [Project Kickoff Email (example)](templates/EXAMPLE-project-kickoff-email.md) | A short email that confirms the project goal, kickoff details, and next action. |  | 2026-08-11 |

## Decision

| Name | What it covers | Owner | Updated |
|---|---|---|---|
| [Refund Requests (example)](decisions/EXAMPLE-refund-requests.md) | How we respond when a client asks for money back after work has started. |  | 2026-08-11 |
